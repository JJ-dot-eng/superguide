import { factionGuides, factionSides, factionCheckedAt, factionAimTargets, factionTactics } from './faction-data.js?v=faction-audit-1';
...
export function factionRecommendation(unit, choice) {
  ...
  const targets = factionAimTargets[`${enemy.id}:${choice}`] || unit.targets;
  ...
  return { enemy, weapon, profile, mode, row, reference, alternatives: candidates.slice(1), tactic: factionTactics[`${enemy.id}:${choice}`] };
}
...
export function renderFactionGuide(guide, stratagems, wikiIcons) {
  const cards = guide.units.map(unit => {
    ...
    const weapons = unit.weapons.map(choice => {
      const { weapon, profile, mode, row, reference, alternatives, tactic } = factionRecommendation(unit, choice);
