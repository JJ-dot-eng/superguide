...
const soloMode = weaponProfiles['solo-silo'].modes[0];
const soloMatchup = calculateMatchup(vox, soloMode);
const beforeSoloSummary = structuredClone(soloMatchup);
const soloSummary = combatSummary(vox, soloMode, soloMatchup, { weapon: 'solo-silo' });
assert.equal(soloSummary.title, '몸통 명중 시 1발 처치 가능 · 위키 기준');
assert.equal(soloSummary.reference.source, `${vox.source}#Tactical_Information`);
assert.equal(soloSummary.reference.checkedAt, '2026-09-16');
assert.deepEqual(soloMatchup, beforeSoloSummary, 'Solo Silo tactics must preserve the two independent explosion calculations');
assert.equal(soloMatchup.best.hits, 3);
