...
assert.equal(openingRouteNote(chargedFactory), '1발 환풍구 안쪽 폭발 가능');
...
assert.match(card(epochFactory), /2발 체력 파괴 가능 \/ 1발 환풍구 안쪽 폭발 가능/);
assert.doesNotMatch(card(epochFactory), /demolition-aim-note|별도 입구 철거|위 탄수와 별도/);
...
assert.match(card(c4Ship), /1개 체력 파괴 가능 \/ 1개 열린 출입구 안쪽 폭발 가능 \(보호막 제거 후\)/);
