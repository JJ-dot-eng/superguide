...
function filteredItems() {
  const matchesSearch = createSearchMatcher(state.search);
  return stratagems.filter(item => (state.category === 'all' || item.category === state.category) && matchesPenetration(item) && matchesSearch(item));
}
function card(item) {
